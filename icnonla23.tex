...
\begin{frame}{A new Byrd-Omojokun approach}
    ...
    \footnotetext{See \cite[\S 15.4.4]{Conn_Gould_Toint_2000}.}
\end{frame}
...
\begin{frame}{A new Byrd-Omojokun approach (cont'd)}
    ...
    \begin{itemize}
        ...
        \begin{align*}
            \min_{\step \in \R^n}   & \quad \big[ \gradobjm[k] ( \iter[k] ) + \hesslagm[k][\iter][\iter] ( \iter[k], \lmub[k] ) \nstep[k] \big]^{\T} \step + \frac{1}{2} \step^{\T} \hesslagm[k][\iter][\iter] ( \iter[k], \lmub[k] ) \step\\
            \textrm{s.t.}           & \quad \jaccubm[k] ( \iter[k] )^{\T} \step \le \alert{\only<1>{0}\only<2>{[\cubm[k](\iter[k]) + \jaccubm[k] ( \iter[k] ) \nstep[k]]^-}},\\
                                    & \quad \xl \le \iter[k] + \nstep[k] + \step \le \xu,\\
...
\begin{frame}{A crucial difficulty in the implementation}
    \begin{itemize}
        ...
        \textcolor{FernGreen}{A well-known approach: a geometry-improving mechanism.\footnote{See \cite{Conn_Scheinberg_Vicente_2008a,Conn_Scheinberg_Vicente_2008b,Fasano_Morales_Nocedal_2009}.}}
    \end{itemize}
